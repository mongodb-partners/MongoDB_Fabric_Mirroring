...
2. Azure Storage explorer is your point to start the troubleshooting. Use below files that start with an underscore to get vital information. (They are not copied to OneLake as they start with underscore"_"). Also note these are pickle files and you can view them using command "python -mpickle _maxid.pkl” in terminal.      
   a. _max_id file: Will tell you what was the maximum _id field that was captured before initial sync began (within the init snapshot). All records with `_id <= this _max_id` as of cluster time N are copied as part of initial_sync.
   b. _init_cluster_time: BSON Timestamp **N** captured at init start. Init sync reads are a snapshot at N; the change stream starts at **N+1** until a resume token exists.
